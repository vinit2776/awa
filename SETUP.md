...
- **Email** — `RESEND_API_KEY` / `EMAIL_FROM`, from a [Resend](https://resend.com) account. `EMAIL_FROM` must use a domain verified in the Resend dashboard (`Name <address@your-domain.com>`); an unverified sender is the usual cause of a 403 on send. With either variable missing, `db/email.ts` logs what it would have sent and returns a result rather than throwing — unconfigured is a supported state, not an error. Two deliberate safeguards: sends are suppressed entirely under vitest so a key in the environment can't turn a test run into live mail at fixture addresses, and a send is never allowed to throw, because `notifyUser()` runs inside the caller's `withTenant` transaction and an exception there would roll back the approval or ticket that triggered the notification. A 5-second timeout bounds how long a slow provider can hold that transaction (and its pooled connection) open; an outbox table is the proper fix once volume justifies it.
...
2. The contact requests a sign-in link at `/vendor-portal/login`. A signed, 15-minute-lived token is emailed via `db/email.ts`; with `RESEND_API_KEY`/`EMAIL_FROM` unset it logs the link to the console instead, so the portal stays usable locally without an account. The result is never surfaced to the sign-in page — it renders the same "check your inbox" response whether or not the address matched, since doing otherwise would leak which emails are registered.
